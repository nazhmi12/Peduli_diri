...
$nik = $this->request->getVar('nik');
$password = sha1($this->request->getVar('password'));
$data = file("config.txt", FILE_IGNORE_NEW_LINES); foreach
($data as $value) {
$pisah = explode("|", $value);
if ($nik == $pisah['0'] && $password == $pisah['2']) { $cek =
TRUE;
} else {
$cek = FALSE;
}
...
session()->set('berhasil', "Login Berhasil"); $session
= session();
$session->markAsFlashdata('berhasil');
return redirect()->to(base_url('dashboard')); } else {
session()->set('failed', "Username / Password Salah"); $session
= session();
